...
@property (nonatomic, copy) void(^getMessageHandler)(NSString *message);
...
{
    //1.补充遮罩
    UIView*mask=[[UIView alloc]initWithFrame:CGRectMake(0, _maskView.sd_y + _maskView.sd_height, self.view.sd_width, self.view.sd_height - _maskView.sd_y - _maskView.sd_height - self.view.sd_height * 0.1)];
    ...
    tipLabel.text = NSLocalizedString(@"QRCODE_TIPS_MESSAGE", @"将取景框对准二维码，即可自动扫描");
...
- (void)setupMaskView
{
    UIView *mask = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, self.view.sd_width, self.view.sd_height * 0.9)];
    _maskView = mask;
...
- (void)setupBottomBar
{
    ...
    [cancelButton setTitle:NSLocalizedString(@"CANCEL_BUTTON_TITLE", @"") forState:UIControlStateNormal];
    [cancelButton sizeToFit];
    cancelButton.frame = CGRectMake(15, (bottomBar.sd_height - cancelButton.sd_height - 5) / 2, cancelButton.sd_width + 20, cancelButton.sd_height + 10);
    [cancelButton addTarget:self action:@selector(cancelButtonClickHandler:) forControlEvents:UIControlEventTouchUpInside];
    [bottomBar addSubview:cancelButton];
    
    //3.闪光灯
    UIButton * flashBtn=[UIButton buttonWithType:UIButtonTypeCustom];
    [flashBtn setBackgroundImage:[UIImage imageNamed:NSLocalizedString(@"qrcode_scan_btn_flash_nor", @"")] forState:UIControlStateNormal];
    [flashBtn setBackgroundImage:[UIImage imageNamed:NSLocalizedString(@"qrcode_scan_btn_flash_down", @"")] forState:UIControlStateSelected];
    flashBtn.contentMode=UIViewContentModeScaleAspectFit;
    flashBtn.frame = CGRectMake(bottomBar.sd_width - 55, (bottomBar.sd_height - 49) / 2, 35, 49);
    ...
    [albumBtn setBackgroundImage:[UIImage imageNamed:NSLocalizedString(@"qrcode_scan_btn_photo_nor", @"")] forState:UIControlStateNormal];
    albumBtn.contentMode=UIViewContentModeScaleAspectFit;
    albumBtn.frame = CGRectMake(flashBtn.sd_x - 20 - 35, (bottomBar.sd_height - 49) / 2, 35, 49);
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    if (metadataObjects.count > 0)
    {
        ...
        if (self.getMessageHandler)
        {
            self.getMessageHandler (metadataObject.stringValue);
        }
    }
}

- (void)onGetMessage:(void (^)(NSString *))handler
{
    self.getMessageHandler = handler;
}
...
- (void)myAlbum
{
    ...
    {
        UIAlertView * alert = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"TIPS_ALERT_TITLE", @"Tips")
                                                        message:NSLocalizedString(@"DEVICE_DOES_NOT_SUPPORT_ALBUM_FEATURE_MESSAGE", @"设备不支持访问相册，请在设置->隐私->照片中进行设置！")
                                                       delegate:nil
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        if (features.count >=1)
        {
            ...
            if (theVC.getMessageHandler)
            {
                theVC.getMessageHandler (scannedResult);
            }
          
        }
        else
        {
            UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"TIPS_ALERT_TITLE", @"Tips")
                                                                message:NSLocalizedString(@"HAS_NOT_A_QRCODE_MESSAGE", @"该图片没有包含一个二维码！") 
                                                               delegate:nil
                                                      cancelButtonTitle:NSLocalizedString(@"KONWN_BUTTON_TITLE", @"知道了")
                                                      otherButtonTitles:nil, nil];
            [alertView show];
